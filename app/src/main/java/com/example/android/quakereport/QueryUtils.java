...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            JSONObject JO_ROOT = new JSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray eqArray = JO_ROOT.getJSONArray("features");

            for (int i = 0; i < eqArray.length(); i++)
            {
                JSONObject currEQ = eqArray.getJSONObject(i);
                JSONObject properties = currEQ.getJSONObject("properties");

                double mag = properties.getDouble("mag");
                //Date date = new Date((long)properties.getInt("time"));
                //SimpleDateFormat dateFormatter = new SimpleDateFormat("MMM DD, yyyy");


              /*  Calendar mydate = Calendar.getInstance();
                mydate.setTimeInMillis(properties.getInt("time")*1000);
                String date = mydate.get(Calendar.DAY_OF_MONTH)+"."+mydate.get(Calendar.MONTH)+"."+mydate.get(Calendar.YEAR);*/
                String place = properties.getString("place");

                earthquakes.add(new Earthquake(mag,(long)properties.getInt("time"), place));
